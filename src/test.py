...
def repeatfunc( args ):
    while True:
        ...
        print(f'All process is done in {end - start:.3f} sec!')
        print('********************************************')
#---------------------------------------------------------
...
if __name__ == '__main__':
    def get_args():
        ...
        parser.add_argument('-a', '--address', type=str, default='192.168.0.101',
                            metavar='address', help='(str)address for connecting to server(default: None)')
        parser.add_argument('-f', '--filename', type=str, default='output.pcap',
                            metavar='filename', help='(str)filename to receive(default: output.pcap)')
...
if __name__ == '__main__':
    ...
    clientSock.connect((args.address, 9090))

    print('연결에 성공했습니다.')
    filename = args.filename
    clientSock.sendall(filename.encode('cp949'))
    print('filename : %s' %filename)
    print('-------------------Ready for Running---------------------')

    func0 = repeatfunc( args )
    func1 = local2nex.local2nex( clientSock, filename )
